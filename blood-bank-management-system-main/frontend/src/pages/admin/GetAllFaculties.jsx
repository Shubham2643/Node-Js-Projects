...
function GetAllFacilities() {
  ...
  // Faculty status and types for filters
  const FacultyTypes = ['hospital', 'blood-lab'];
  ...
  const fetchAllFacilities = async (showToast = false) => {
    try {
      ...
      toast.error(error.message || "Failed to load Faculty data.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const filteredFacilities = facilities
    .filter(Faculty => {
      const matchesSearch = !filters.search || 
        ...
        Faculty.email?.toLowerCase().includes(filters.search.toLowerCase()) ||
        Faculty.registrationNumber?.toLowerCase().includes(filters.search.toLowerCase()) ||
        Faculty.phone?.includes(filters.search);
      
      const matchesType = filters.FacultyType === 'all' || Faculty.FacultyType === filters.FacultyType;
    ...
    .sort((a, b) => {
      ...
      switch (filters.sortBy) {
        ...
        case 'status':
          ...
          break;
        case 'type':
          aValue = a.FacultyType?.toLowerCase();
          bValue = b.FacultyType?.toLowerCase();
          break;
        default:
          aValue = a.name?.toLowerCase();
          bValue = b.name?.toLowerCase();
  ...
  const hospitalCount = facilities.filter(f => f.FacultyType === 'hospital').length;
  const labCount = facilities.filter(f => f.FacultyType === 'blood-lab').length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-red-100 p-6 mb-6">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <select
              ...
              onChange={(e) => setFilters(prev => ({ ...prev, FacultyType: e.target.value }))}
        ...
        {pendingCount > 0 && (
          <div className="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-amber-800">{pendingCount} Faculty Approval{pendingCount !== 1 ? 's' : ''} Pending</p>
                <p className="text-amber-700 text-sm">
                  {pendingCount} medical Faculty{pendingCount !== 1 ? 's' : ''} awaiting administrative review and approval
                </p>
              </div>
            </div>
          </div>
        ...
        {filteredFacilities.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl shadow-lg border border-red-100">
            ...
            <p className="text-gray-600">
              {facilities.length === 0 
                ? 'The medical Faculty database is currently empty.' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredFacilities.map((Faculty) => (
              <div
                key={Faculty._id}
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4 pb-4 border-b border-gray-100">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 mt-1 line-clamp-1">{Faculty.email}</p>
                  </div>
                  <div className="flex flex-col gap-2 items-end">
                    {getStatusBadge(Faculty.status)}
                    {getTypeBadge(Faculty.FacultyType)}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="text-gray-700 font-medium">Reg: {Faculty.registrationNumber}</span>
                  </div>
                  
                  <div className="flex items-center gap-3 text-sm">
                    <Phone className="w-4 h-4 text-red-500 flex-shrink-0" />
                    <span className="text-gray-700">{Faculty.phone || 'Not provided'}</span>
                  </div>
                  
                  <div className="flex items-center gap-3 text-sm">
                    <Briefcase className="w-4 h-4 text-red-500 flex-shrink-0" />
                    <span className="text-gray-700 capitalize">{Faculty.FacultyCategory || 'General'}</span>
                  </div>

                  {/* Operational Status */}
                  <div className="flex items-center gap-3 text-sm">
                    <Clock className={`w-4 h-4 flex-shrink-0 ${
                      Faculty.is24x7 ? 'text-green-500' : 'text-gray-500'
                    }`} />
                    <span className="text-gray-700 font-medium">
                      {Faculty.is24x7 ? '24/7 Service Available' : `Hours: ${Faculty.operatingHours?.open || 'N/A'} - ${Faculty.operatingHours?.close || 'N/A'}`}
                    </span>
                  </div>

                  {Faculty.emergencyServices && (
                    <div className="flex items-center gap-3 text-sm">
                  ...
                  <div className="flex items-start gap-3 text-sm pt-2 border-t border-gray-100">
                    ...
                    <div className="text-gray-700 line-clamp-2">
                      {Faculty.address?.street && `${Faculty.address.street}, `}
                      {Faculty.address?.city}, {Faculty.address?.state}
                      {Faculty.address?.pincode && ` - ${Faculty.address.pincode}`}
                    </div>
                  </div>
                </div>
              </div>
